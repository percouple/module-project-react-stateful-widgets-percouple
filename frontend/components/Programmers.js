...
export const listOfAwesome = [
  ...
  { id: '7', name: 'Steve Stevenson'},
  { id: '8', name: 'Terry Bradshaw'},
];

export default function Programmers() {
  // We'll have to use the state hook twice, as we need two slices of state.
  // The programmers list on the one hand, and the id of the featured programmer on the other.
  let [programmersList, setProgrammersList] = useState(listOfAwesome); 
  let [featuredProgrammerID, setFeaturedProgrammerID] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    const featuredProgrammer = programmersList.find((programmer) => featuredProgrammerID === programmer.id);
    return featuredProgrammer ? featuredProgrammer.name : null;
      ...
      <div id='featured' style={style}>
        {
          ...
          featuredProgrammerID
            ? `🎉 Let's celebrate ${getNameOfFeatured(featuredProgrammerID)}! 🥳`
